...
setup_user_and_dirs() {
  ...
  if ! id "$BENISPLOY_USER" >/dev/null 2>&1; then
    ...
    # Make sure an existing user has a usable shell
    usermod -s /bin/bash "$BENISPLOY_USER"
    log "Updated shell for existing user: $BENISPLOY_USER"
  fi

  # Allow the user to use Docker
  if getent group docker >/dev/null 2>&1; then
    usermod -aG docker "$BENISPLOY_USER"
    log "Added $BENISPLOY_USER to docker group"
  fi
  ...
  # Ownership
  chown -R \
    "${BENISPLOY_USER}:${BENISPLOY_GROUP}" \
    "$APPS_DIR" \
    "$BIN_DIR" \
    "$TRAEFIK_DIR" \
    "$AUTHORIZED_KEYS_DIR"
  ...
  # Make sure the user's home directory has the correct ownership
  chown "${BENISPLOY_USER}:${BENISPLOY_GROUP}" "$BENISPLOY_BASE"
  chmod 755 "$BENISPLOY_BASE"

  log "Directory structure created under $BENISPLOY_BASE"
  log "SSH authorized_keys: $AUTHORIZED_KEYS_FILE"
}
